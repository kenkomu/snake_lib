...
pub fn render() {
  GAME.with(|game| {
      ...
      // Display the points at the top of the grid
      let points_element = document
          .create_element("div")
          .unwrap_throw()
          .dyn_into::<HtmlDivElement>()
          .unwrap_throw();

      points_element.set_class_name("points");
      points_element.set_inner_text(&format!("Points: {}", game.points));
      root_container.append_child(&points_element).unwrap_throw();
